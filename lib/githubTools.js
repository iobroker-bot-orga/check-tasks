...
const context = {};
const fileHistoryCommitCache = new Map();
const fileHistorySnapshotCache = new Map();
...
async function getFileEntryAddedDate(owner, repository, path, key) {
    ...
    const fileCachePrefix = `${owner}/${repository}/${path}`;
    ...
    async function latestCommitBefore(untilIso) {
        const commitCacheKey = `${fileCachePrefix}::${untilIso || 'HEAD'}`;
        if (fileHistoryCommitCache.has(commitCacheKey)) {
            return fileHistoryCommitCache.get(commitCacheKey);
        }
        ...
        fileHistoryCommitCache.set(commitCacheKey, result);
        return result;
    }

    // is `key` present as a top level property of the file at commit `sha`?
    async function keyPresentAt(sha) {
        const snapshotCacheKey = `${fileCachePrefix}::${sha}`;
        let snapshot = fileHistorySnapshotCache.get(snapshotCacheKey);
        if (!snapshot) {
            ...
            try {
                snapshot.json = JSON.parse(response.data);
            } catch {
                snapshot.json = null;
            }
            fileHistorySnapshotCache.set(snapshotCacheKey, snapshot);
        }

        if (snapshot.json) {
            return Object.prototype.hasOwnProperty.call(snapshot.json, key);
        }
        return snapshot.raw.includes(`"${key}":`);
    }
